...
from api.sync.import_event_pack import import_event_pack


class Command(BaseCommand):
    help = 'Import an event pack JSON payload into the local database.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            result = import_event_pack(payload)
        except Exception as exc:
            if isinstance(exc, CommandError):
                raise
            raise CommandError(str(exc)) from exc

        self.stdout.write(self.style.SUCCESS(f"Imported event pack for event {result['event_id']}"))
