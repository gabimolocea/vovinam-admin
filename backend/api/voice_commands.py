"""Comenzile vocale de arbitraj: vocabularul si potrivirea lor.

Nu dictare libera. Fiecare rostire se potriveste cu una din comenzile
cunoscute, sau cu niciuna - iar "cu niciuna" e un rezultat bun. O
comanda inteleasa gresit schimba scorul unui meci; una respinsa doar
cere repetarea. Tot ce urmeaza e construit in jurul asimetriei asteia.

Vocabularul si gardile au iesit din masuratori pe prototip
(prototypes/voice-commands), nu din presupuneri: cuvintele scurte
rostite singure se transcriu prost, iar "minus" e auzit constant
"minuz" - ceea ce la prima incercare a produs exact eroarea periculoasa,
un punct adaugat in loc de scazut.
"""
...
# Cuvantul de activare. Arbitrul central vorbeste tot meciul cu
# sportivii si cu masa - fara un cuvant care nu apare niciodata natural,
# o fraza obisnuita ar putea deveni comanda. "Arbitru" nu se rosteste in
# timpul unei reprize.
WAKE_WORDS = ['arbitru', 'arbitrul']


def strip_wake_word(transcript):
    """Intoarce (are_cuvant_de_activare, restul rostirii).

    Cuvantul poate sta oriunde in fata: Whisper prinde des si un "asa"
    sau un oftat inainte de el.
    """
    said = normalize(transcript)
    for wake in WAKE_WORDS:
        idx = said.find(wake)
        if idx != -1:
            return True, said[idx + len(wake):].strip()
    return False, said


def interpret(transcript, require_wake_word=True):
    """Ce ar trebui sa faca sistemul cu rostirea asta.

    Intoarce (cod, id_comanda). Codul spune de ce nu s-a intamplat
    nimic, ca interfata sa raspunda altceva la "n-am auzit trezirea"
    decat la "am auzit, dar nu inteleg".
    """
    if not normalize(transcript):
        return ('tacere', None)

    awake, rest = strip_wake_word(transcript)
    if require_wake_word and not awake:
        return ('fara_activare', None)

    cmd = match(rest if awake else transcript)
    if cmd is None:
        return ('neinteles', None)
    return ('ok', cmd)
