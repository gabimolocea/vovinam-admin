...
class RefereePointEventTests(TestCase):
    def setUp(self):
        ...
        self.athlete1 = Athlete.objects.create(first_name='Red', last_name='One')
        self.athlete2 = Athlete.objects.create(first_name='Blue', last_name='Two')
        # referees
        self.refs = []
        for i in range(5):
            r = Athlete.objects.create(first_name=f'Ref{i}', last_name='Ref', is_referee=True)
            self.refs.append(r)
        # category and match
